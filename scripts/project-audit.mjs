...
const ALIAS_ROOT = fs.existsSync(path.join(ROOT, 'src')) ? path.join(ROOT, 'src') : ROOT;
const SOURCE_DIRS = fs.existsSync(path.join(ROOT, 'src'))
  ? [path.join(ROOT, 'src')]
...
function resolveImport(fromFile, specifier) {
  ...
  if (specifier.startsWith('@/')) {
    const bases = [path.join(ALIAS_ROOT, specifier.slice(2))];
...
const sourceFiles = SOURCE_DIRS.flatMap((dir) => walk(dir)).filter((file) => EXTENSIONS.has(path.extname(file)));
